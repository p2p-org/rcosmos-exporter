...
pub struct AstriaValidatorInfoScrapper {
...
impl AstriaValidatorInfoScrapper {
    pub fn new(
        client: Arc<BlockchainClient>,
        chain_id: ChainId,
        network: String,
    ...
    async fn get_rpc_validators(&self, path: &str) -> anyhow::Result<Vec<TendermintValidator>> {
        info!("(Astria Validator Info) Fetching RPC validators");
    ...
    async fn get_latest_block(&self) -> anyhow::Result<(String, Vec<String>)> {
        ...
        let signatures = block_response
            ...
            .signatures
            .into_iter()
            .filter(|sig| sig.block_id_flag == 2)
    ...
    async fn process_validators(&mut self) -> anyhow::Result<()> {
        ...
        info!("(Astria Validator Info) Processing RPC validators");
        for validator in rpc_validators {
            ...
            if !signed_validators.contains(&address) {
                info!(
                    "(Astria Validator Info) Validator {} missed block {}",
            ...
            // For Astria, we'll use the address as both name and address since we don't have moniker
...
#[async_trait]
impl Task for AstriaValidatorInfoScrapper {
    async fn run(&mut self) -> anyhow::Result<()> {
        self.process_validators().await
    }

    fn name(&self) -> &'static str {
        "Astria Validator Info Scrapper"
    }
}
