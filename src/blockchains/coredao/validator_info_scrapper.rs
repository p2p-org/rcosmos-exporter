...
use crate::{
    blockchains::coredao::{
        metrics::{COREDAO_VALIDATORS, COREDAO_VALIDATOR_JAILED, 
                 COREDAO_VALIDATOR_SLASH_COUNT, COREDAO_VALIDATOR_SLASH_BLOCK},
    },
...
impl CoreDaoValidatorInfoScrapper {
    ...
    async fn get_validators(&self) -> Vec<String> {
        ...
        let res = match self.client.post_json("", &payload).await {
            ...
            Err(e) => {
                error!("(Core DAO Validator Info) Error calling validators endpoint: {}", e);
                return Vec::new();
            }
        };
        ...
        info!("(Core DAO Validator Info) Found {} validators", validators.len());
        validators
    }
    ...
    async fn get_all_candidates(&self) -> Vec<String> {
        ...
        let res = match self.client.post_json("", &payload).await {
            ...
            Err(e) => {
                error!("(Core DAO Validator Info) Error calling all candidates endpoint: {}", e);
                return Vec::new();
            }
        };
        ...
        let result: Value = match serde_json::from_str(&res) {
            Ok(val) => val,
            Err(e) => {
                error!("(Core DAO Validator Info) Error parsing JSON for all candidates: {}", e);
                return Vec::new();
            }
        };
        ...
        info!("(Core DAO Validator Info) Found {} candidates", candidates.len());
        candidates
    }
    
    async fn check_if_jailed(&self, validator_address: &str) -> bool {
        info!("(Core DAO Validator Info) Checking if validator {} is jailed", validator_address);
        
        // Use contract CandidateHub.sol using function isJailed(address)
        let data = format!("0x14bfb527000000000000000000000000{}", validator_address.trim_start_matches("0x"));
        let payload = json!({
            "jsonrpc": "2.0",
            "method": "eth_call",
            "params": [{
        ...
        info!("(Core DAO Validator Info) Jail check response for target validator: {}", res);
    ...
    async fn check_slash_info(&self, validator_address: &str) -> (i64, i64) {
        info!("(Core DAO Validator Info) Checking slash info for validator {}", validator_address);
        
        // Use contract SlashIndicator.sol with function getSlashIndicator(address)
        let data = format!("0x37c8dab9000000000000000000000000{}", validator_address.trim_start_matches("0x"));
        let payload = json!({
            "jsonrpc": "2.0",
            "method": "eth_call",
            "params": [{
        ...
        let res = match self.client.post_json("", &payload).await {
            ...
            Err(e) => {
                error!("(Core DAO Validator Info) Error checking slash info for {}: {}", validator_address, e);
                return (-1, -1);
            }
        };
        ...
        let result: Value = match serde_json::from_str(&res) {
            Ok(val) => val,
            Err(e) => {
                error!("(Core DAO Validator Info) Error parsing JSON for slash info: {}", e);
                return (-1, -1);
            }
        };
        ...
        if let Some(error) = result.get("error") {
            error!("(Core DAO Validator Info) RPC error in slash info response: {:?}", error);
        ...
        let result_hex = match result.get("result") {
            Some(Value::String(hex)) => {
                ...
                info!("(Core DAO Validator Info) Extracted hex result: {}", trimmed);
                trimmed
            },
            Some(other) => {
                error!("(Core DAO Validator Info) Unexpected result type for slash info: {:?}", other);
                return (-1, -1);
            },
            None => {
        ...
        if result_hex.is_empty() {
            info!("(Core DAO Validator Info) Empty slash info result for {} - likely not slashed", validator_address);
            return (-1, -1);
        }
        ...
        if result_hex == "0x" || result_hex.len() < 128 {
            info!("(Core DAO Validator Info) No slash info for validator {}", validator_address);
            return (-1, -1);
        }
        ...
        let block_height = match i64::from_str_radix(block_height_hex, 16) {
            Ok(val) => val,
            Err(e) => {
                error!("(Core DAO Validator Info) Error parsing block height hex: {}", e);
                -1
            }
        };
        ...
        let slash_count = match i64::from_str_radix(slash_count_hex, 16) {
            Ok(val) => val,
            Err(e) => {
                error!("(Core DAO Validator Info) Error parsing slash count hex: {}", e);
                -1
            }
        };
        
        info!("(Core DAO Validator Info) Parsed slash info - Block Height: {}, Slash Count: {}", 
...
impl Task for CoreDaoValidatorInfoScrapper {
    async fn run(&mut self, delay: Duration) {
        ...
        info!("(Core DAO Validator Info) Found {} active validators", active_validators.len());
        
        // Get all candidates (including inactive validators) and normalize to lowercase
        let all_candidates: Vec<String> = self.get_all_candidates().await
            .into_iter()
            .map(|addr| addr.to_lowercase())
            .collect();
        
        info!("(Core DAO Validator Info) Found {} total candidates", all_candidates.len());
        ...
        for validator in &all_validators {
            ...
            info!("(Core DAO Validator Info) Setting validator metric for: {} (active: {})", 
                  validator, is_active);
